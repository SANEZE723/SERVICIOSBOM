...
import { CalendarIcon, BookOpenIcon, DownloadIcon, ClockIcon, ClipboardListIcon, RefreshIcon, EyeIcon, EyeOffIcon, UploadIcon, QuestionMarkCircleIcon, BookmarkIcon, LogoutIcon, TrashIcon } from './components/icons';
...
const initializeSchedule = (): Schedule => {
  ...
  try {
    ...
    if (savedScheduleJSON) {
      ...
      if (
        ...
        parsedData.hasOwnProperty('commandStaff') &&
        Array.isArray(parsedData.commandStaff)
      ) {
        scheduleToLoad = parsedData;
      } else {
        console.warn("Saved schedule data is invalid. Falling back to default.");
  ...
  if (dataCopy.services && !dataCopy.hasOwnProperty('sportsEvents')) {
    dataCopy.sportsEvents = (dataCopy.services || []).filter((s: Service | null) => s && s.title && s.title.toUpperCase().includes('EVENTO DEPORTIVO'));
    dataCopy.services = (dataCopy.services || []).filter((s: Service | null) => s && s.title && !s.title.toUpperCase().includes('EVENTO DEPORTIVO'));
  }

  // Ensure top-level arrays exist and are valid arrays
  if (!Array.isArray(dataCopy.services)) dataCopy.services = [];
  if (!Array.isArray(dataCopy.sportsEvents)) dataCopy.sportsEvents = [];
  if (!Array.isArray(dataCopy.commandStaff)) dataCopy.commandStaff = [];
  
  let idCounter = 0;
  const processServices = (services: any[]): Service[] => {
    if (!Array.isArray(services)) return [];
    
    return services
      .filter((s): s is Service => s && typeof s === 'object' && typeof s.title === 'string')
      .map((service: Service) => {
        ...
        if (Array.isArray(newService.assignments)) {
          newService.assignments = newService.assignments
            .filter((a): a is Assignment => a && typeof a === 'object' && a.location && a.time && a.personnel)
            .map((assignment: Assignment) => {
              const newAssignment = { ...assignment };
              if (!newAssignment.id) newAssignment.id = `assign-hydrated-${Date.now()}-${idCounter++}`;
  ...
  dataCopy.services = processServices(dataCopy.services);
  dataCopy.sportsEvents = processServices(dataCopy.sportsEvents);

  dataCopy.commandStaff = dataCopy.commandStaff
    .filter((o): o is Officer => o && typeof o === 'object' && o.role && o.name)
    .map((officer: Officer) => {
      const newOfficer = { ...officer };
      if (!newOfficer.id) newOfficer.id = `officer-hydrated-${Date.now()}-${idCounter++}`;
      return newOfficer;
    });

  return dataCopy as Schedule;
};
...
const App: React.FC = () => {
  ...
  const handleDeleteSelectedServices = () => {
    ...
    const serviceCount = selectedServiceIds.size;
    const serviceNoun = serviceCount === 1 ? 'servicio' : 'servicios';
  
    if (
      window.confirm(
        `¿Estás seguro de que quieres eliminar permanentemente ${serviceCount} ${serviceNoun} seleccionado(s)? Esta acción no se puede deshacer.`
      )
    ) {
      setSchedule((currentSchedule) => {
        // Guard against schedule being null or undefined
        if (!currentSchedule) {
            return currentSchedule;
        }
  
        // Defensively filter services, ensuring each service `s` is an object with an `id`
        const updatedServices = (currentSchedule.services || []).filter(
          (s) => s && s.id && !selectedServiceIds.has(s.id)
        );
        const updatedSportsEvents = (currentSchedule.sportsEvents || []).filter(
          (s) => s && s.id && !selectedServiceIds.has(s.id)
        );
        
        // Return the new state object with cleaned service lists
        return {
          ...currentSchedule,
      ...
      // Clear the selection and show a confirmation message
      setSelectedServiceIds(new Set());
      setToastMessage(
        `${serviceCount} ${serviceNoun} eliminado(s) permanentemente.`
      );
    }
  };
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button onClick={handleExportByTime} className="flex items-center gap-2 px-4 py-2 rounded-md bg-teal-600 hover:bg-teal-500 text-white font-medium transition-colors"><DownloadIcon className="w-5 h-5" />Exportar por Hora</button>
                 {selectedServiceIds.size > 0 && view === 'schedule' && (
                  <>
                    <button onClick={handleDeleteSelectedServices} className="flex items-center gap-2 px-4 py-2 rounded-md text-white font-medium transition-colors animate-fade-in bg-red-800 hover:bg-red-700">
                      <TrashIcon className="w-5 h-5" />
                      Eliminar ({selectedServiceIds.size})
                    </button>
                    <button onClick={handleToggleVisibilityForSelected} className={`flex items-center gap-2 px-4 py-2 rounded-md text-white font-medium transition-colors animate-fade-in ${visibilityAction.action === 'hide' ? 'bg-yellow-600 hover:bg-yellow-500' : 'bg-purple-600 hover:bg-purple-500'}`}>
                 ...
                 <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 rounded-md bg-red-600 hover:bg-red-700 text-white font-medium transition-colors">
